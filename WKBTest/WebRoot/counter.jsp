<%@ page language="java" import="java.util.*" pageEncoding="utf-8"%>
<%
String path = request.getContextPath();
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
%>
<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<title>Document</title>
	<link rel="img/LOGO.ico" href="">
	<link rel="stylesheet" href="css/Adaptive.css">
	<link rel="stylesheet" href="css/counter.css">
	<link rel="stylesheet" href="http://at.alicdn.com/t/font_518775_28rxw0wmc2yjsjor.css">
	<script src="js/method.js"></script>
	<script src="js/counter.js"></script>
</head>
<body>
	<header>
		<div class="name col-lg-10 col-md-10 col-sm-10 col-xs-10">
			<a href="http://qm.qq.com/cgi-bin/qm/qr?k=aB9H7UnPWVmgrhgFyUv3nyZXXw8NpSdt">点击加QQ群：链克涛哥交流群
			</a>
		</div>
	</header>
	<nav>
		<div class="con col-lg-10 col-md-10 col-sm-10 col-xs-10 col-sj-10">
			<div class="logo col-lg-2 col-md-2 col-sm-2 col-xs-2 col-sj-2">
				<a href="">
				
				<img src="img/LOGO.ico" alt="" width="46px" height="46px">

				</a>
			</div>
			<ul class="col-lg-10 col-md-10 col-sm-10 col-xs-10 col-sj-10">
					<li><a href="<%=request.getContextPath()%>/DateServlet">实时行情</a></li>
				<li><a
					href="<%=request.getContextPath()%>/DateServlet?method=Info">矿场情况</a></li>
				<li><a href="<%=path %>/counter.jsp" class="current">矿力计算</a></li>
				<li><a
					href="<%=request.getContextPath()%>/QueryTrendServlet?method=queryDe&address=0x0003fadb6afb6885e0634366a31552cab47e5599">区块查询</a></li>
				<li><a
					href="<%=request.getContextPath()%>/QKServlet?method=top"
					class="current">口袋排行</a></li>
				<li><a href="">链克论坛</a></li>
			</ul>
		</div>
	</nav>
	<img src="img/banner1.jpg" alt="" class="banner">
	<div class="content col-lg-10 col-md-10 col-sm-10 col-xs-10 col-sj-10">
		<div class="top">
			<h3>玩客云挖矿收益计算器</h3>
			<p class="facility">设备：玩客云WS1608</p>
			<p class="facility-2">
				矿机能力值=1，CPU因子权重=20 
				<br>
				内存因子权重=10，存储因子权重=5
			</p>
		</div>
		<ul>
			<li>
				<p class="nums">142.1</p>
				<p class="num">矿机分数</p>
			</li>
			<li>
				<p class="nums">4.4</p>
				<p class="num">预计得币个数</p>
			</li>
			<li>
				<p class="nums nums-red">17.81</p>
				<p class="num">预计收益CNY/天</p>
			</li>
		</ul>
		<ul>
			<li class="plan-left">
				<p class="plan-left-top">上行宽带</p>
				<p class="plan-left-bot">15Mbps</p>
			</li>
			<li class="plan-right">
				<span>1</span>
				<div class="scroll">
					<div class="bar"></div>
					<div class="mask"></div>
				</div>
				<span>100</span>
			</li>
		</ul>
		<ul>
			<li class="plan-left">
				<p class="plan-left-top">存储空间</p>
				<p class="plan-left-bot">1200GB</p>
			</li>
			<li class="plan-right">
				<span>1</span>
				<div class="scroll  scroll-2">
					<div class="bar"></div>
					<div class="mask"></div>
				</div>
				<span>2000</span>
			</li>
		</ul>
		<ul>
			<li class="plan-left">
				<p class="plan-left-top">在线时长</p>
				<p class="plan-left-bot">22小时</p>
			</li>
			<li class="plan-right">
				<span>1</span>
				<div class="scroll scroll-3">
					<div class="bar"></div>
					<div class="mask"></div>
				</div>
				<span>24</span>
			</li>
		</ul>
	</div>
	<footer>
		<div class="footer-bottom col-lg-10 col-md-10 col-sm-10 col-xs-10 col-sj-10">
			<span>Copyright©2017 xycdn.All Rights Reserved 山西省易科科技有限公司版权所有 晋B2-20050219-1</span>
			<span class="footers-b">|</span>
			<a href="" class="footers">
			关于我们</a>
			<span class="footers-b">|</span>
			<a href="" class="footers">
			联系我们</a>
		</div>
	</footer>
</body>
</html>
